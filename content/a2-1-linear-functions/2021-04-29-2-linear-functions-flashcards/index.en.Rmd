...
for m in m_list:
    CTNS_MAKE(target=m)
    CTNS_BUILD(target=m, id='quiz_set_1', write_image=False, encrypt=False, opt_ctns = [ 
        "flashcard_quiz"
       ])

m_list = [
    ['LF_PaLineS', 'LF_PeLineS', 'LF_PointSlopeFormS', 'LF_SlopeFormulaS']
]

for m in m_list:
    CTNS_MAKE(target=m)
    CTNS_BUILD(target=m, id='quiz_set_2', write_image=False, encrypt=False, opt_ctns = [ 
        "flashcard_quiz"
       ])
```

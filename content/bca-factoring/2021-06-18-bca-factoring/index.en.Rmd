...
m_list = \
  ...
  + [ { 'name': s, 'qset':[s] , 'options':CTNS.FORM_TEXT_1+['body_class="ctns-no-color-group-message"', 'color_group_a'] } for s in ['rational_functionD8_5a'] ]
...
```{python, eval=TRUE, results="asis", echo=FALSE, warning=FALSE, message=FALSE}
for m in m_list:
    CTNS_BUILD(target=m, preview=False, process_support_files=False, version=VERSION, write_image=True, encrypt=False)
    #print("Built: %s" % m)
```

...
```{r, echo=FALSE}
blogdown::shortcode_open("spoiler", text="<strong>Strategy: When you see \\(| N |\\) for some number \\(N\\)</strong>", .type="html")
```

1. You know you are working with an absolute value when you see the two vertical bars,

\begin{equation}
\LARGE{ | \ \ \ | }.
\end{equation}

2. The absolute value of a number makes that number positive. 

3. If a number is already positive, then the absolute value of that number is the number itself.

4. If a number is negative, then the absolute value of that number is the opposite (negative) of that number.
